		...
		if ( IsEnabled )
		{
			...
			if ( ImGui::Begin( "Resource Monitor", &IsEnabled ) )
			{
				...
				if ( ImGui::BeginTable( "##ResourceTable", 2, flags, size ) )
				{
					...
					// without referencing the internal data as a pointer or copying the handle
					// this worked fine in my own engine as I could detect a refresh when weak_ptr's became expired...
					// but since your engine resources are manually ref counted it gets weird with ImGui.
