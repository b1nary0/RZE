...
namespace Diotima
{
	...
	void DX12GFXVertexBuffer::Allocate(const std::vector<float>& data)
	{
		...
		memcpy(pVertexDataBegin, data.data(), bufferSize);
		...
		mGPUBufferView->StrideInBytes = sizeof(TempVertexDataLayout);
