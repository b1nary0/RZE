...
namespace Diotima
{
	...
	void GLRenderTargetTexture::Initialize()
	{
		...
		openGL.BindFramebuffer(GetFrameBufferID());
	...
	{
		OpenGLRHI::Get().BindFramebuffer(GetFrameBufferID());
		OpenGLRHI::Get().BindTexture(GL_TEXTURE_2D, GetTextureID());
	}

	void GLRenderTargetTexture::Unbind()
	{
		OpenGLRHI::Get().BindFramebuffer(0);
		OpenGLRHI::Get().BindTexture(GL_TEXTURE_2D, 0);
	}
